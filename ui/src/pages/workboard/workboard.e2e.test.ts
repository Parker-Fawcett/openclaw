...
suite.define(() => {
  ...
  it("collapses empty stages into rails without squeezing active columns", async () => {
    ...
    try {
      ...
      await recorded.page.locator('input[name="workboard-collapse-empty-columns"]').check();
  ...
  it("keeps touch collapse controls visible and at least 44px", async () => {
    const artifacts: ProofArtifacts = { screenshots: [], videos: [] };
    const recorded = await newRecordedPage("workboard-touch-collapse", { hasTouch: true });
    try {
      await installMockGateway(recorded.page, {
        methodResponses: {
          "config.get": workboardConfigSnapshot(),
      ...
      await recorded.page.setViewportSize({ height: 844, width: 390 });
      const response = await recorded.page.goto(`${suite.server.baseUrl}workboard`);
      expect(response?.status()).toBe(200);

      const collapseButton = recorded.page.getByRole("button", { name: "Collapse Review column" });
      ...
      await closeRecordedPage(recorded, artifacts, "workboard-touch-collapse");
    }
  });
